package Bio::KBase::Transform::Service;
...
sub call_method {
    ...
    my $ctx = Bio::KBase::Transform::ServiceContext->new($self->{loggers}->{userlog},
...
{
    # Service Transform requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for Transform but no authentication header was passed");
	}
	...
	my $stderr = Bio::KBase::Transform::ServiceStderrWrapper->new($ctx, $get_time);
	$ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service Transform.");
    }
...
package Bio::KBase::Transform::ServiceContext;

use strict;

=head1 NAME

Bio::KBase::Transform::ServiceContext
...
package Bio::KBase::Transform::ServiceStderrWrapper;
