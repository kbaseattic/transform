...
def transform(workspace_service_url=None, workspace_name=None, object_name=None,
    ...
    logger.info("Starting conversion of KBaseEnigmaMetals.SamplePropertyMatrix to TSV.SampleProperty")
    ...
    if stderr is not None and len(stderr) > 0:
        logger.error("Transformation from KBaseEnigmaMetals.SamplePropertyMatrix to TSV.SampleProperty failed")
        logger.error(stderr)
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    logger = script_utils.stderrlogger(__file__)
    logger.info("Starting download of SamplePropertyMatrix => TSV.SampleProperty")
    try:
