...
def transform(workspace_service_url=None, workspace_name=None, object_name=None,
    ...
    """
    Converts KBaseFeatureValues.FeatureClusters to TSV-formatted file.
    ...
    Returns:
        TSV-formatted file containing data from FeatureClusters object.
    ...
    logger.info("Starting conversion of KBaseFeatureValues.FeatureClusters to TSV")
    ...
    if stderr is not None and len(stderr) > 0:
        logger.error("Transformation from KBaseFeatureValues.FeatureClusters to TSV failed on {0}".format(input_directory))
        logger.error(stderr)
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    logger = script_utils.stderrlogger(__file__)
    logger.info("Starting download of FeatureClusters => TSV")
    try:
