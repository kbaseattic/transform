...
my $tables = {PhenotypeSimulations => [["geneko","mediaws","media","addtlCpd","observed_growth","simulated_growth","simulated_growth_fraction","phenotype_class"]]};
for (my $i=0; $i < @{$obj->{phenotypeSimulations}}; $i++) {
	...
	my $media = $output->[0]->[1];
	push(@{$tables->{PhenotypeSimulations}},[join(";",@{$genekolist}),$mediaws,$media,join(";",@{$cpdlist}),$pheno->{normalizedGrowth},$simpheno->{simulatedGrowth},$simpheno->{simulatedGrowthFraction},$simpheno->{phenoclass}]);
}
