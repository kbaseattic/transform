...
#                  handle_id = None,
    ...
    Args:
        ...
        wsname: Name of ws to load into
        wsurl: URL of WS server instance the WS is on.
    ...
    if input_mapping is None: 
        ...
        fasta_file_name = os.path.join(working_directory,fasta_files[0]) 
 
        if len(fasta_files) > 1: 
            logger.warning("Not sure how to handle multiple FASTA files in this context. Using {0}".format(fasta_file_name)) 
    else: 
        fasta_file_name = os.path.join(os.path.join(working_directory, "FASTA.DNA.Assembly"), simplejson.loads(input_mapping)["FASTA.DNA.Assembly"]) 
