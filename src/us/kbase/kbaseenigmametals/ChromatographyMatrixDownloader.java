...
public class ChromatographyMatrixDownloader {
	...
	public static void main(String[] args) throws Exception {
		...
		ChromatographyMatrixDownloader downloader = new ChromatographyMatrixDownloader();
		downloader.download(args);
	}

	public ChromatographyMatrixDownloader() {
		...
		try {
			...
			if (line.hasOption("help")) {
				...
				if (validateInput(line)) {
		            ...
		            ChromatographyMatrix matrix = client.getObjects(Arrays.asList(new ObjectIdentity().withRef(ref)))
		                    .get(0).getData().asClassInstance(ChromatographyMatrix.class);

					// System.out.println(matrix.toString());
					String outputFileName = line.getOptionValue("of");
			        if (outputFileName == null)
			            outputFileName = "chromatography.tsv";
	...
	private void generateTSV(PrintWriter pw, ChromatographyMatrix matrix) throws Exception {
